...
http_code=$(curl -sS -o /dev/null -w '%{http_code}' \
  -X PUT "${VAULT_URL}/vault/${PRICING_PATH}" \
  -H "Authorization: Bearer ${OBSIDIAN_API_KEY}" \
  -H "Content-Type: text/markdown" \
  -d "$PRICING_CONTENT")

if [[ "$http_code" == "204" || "$http_code" == "200" ]]; then
  log "Pricing report written to vault: ${PRICING_PATH}"
else
  log "Pricing report write failed: HTTP ${http_code}"
fi
...
while IFS= read -r inv; do
  ...
  while IFS= read -r pid; do
    ...
    if (( batch_count >= 100 )); then
      # Fetch product data for this batch
      ...
      # Count products with non-empty links vs empty/missing links
      batch_listed=$(printf '%s' "$batch_result" | jq '[.products // {} | to_entries[] | select(.value.links // {} | length > 0)] | length')
      batch_unlisted=$(printf '%s' "$batch_result" | jq '[.products // {} | to_entries[] | select(.value.links // {} | length == 0)] | length')
      inv_listed=$((inv_listed + batch_listed))
      inv_unlisted=$((inv_unlisted + batch_unlisted))
